...
func (m Model) renderAlarmsPane() string {
    _, _, _, alarms, _, _, _, _, _, _ := m.state.GetSnapshot() // MODIFIED: Replace 'current' with '_' to ignore unused variable
...
func (m Model) renderStatusPane() string {
    _, _, _, _, tx, rx, timing, status, connCount, lastCommand := m.state.GetSnapshot() // MODIFIED: Capture new fields
    content := lipgloss.JoinVertical(lipgloss.Left,
        ...
        statusKeyStyle.Render("Cmd Conns: ")+fmt.Sprintf("%d", connCount), // NEW: Display command connection count
        statusKeyStyle.Render("Last Cmd: ")+lastCommand,                    // NEW: Display last command
...
func (m Model) renderTxRxPane() string {
    _, _, _, _, tx, rx, _, _, _, _ := m.state.GetSnapshot() // MODIFIED: Ignore new fields
    var content strings.Builder
...
func (m Model) renderDataPane() string {
    datastore, prevData, lastChange, _, _, _, _, _, _, _ := m.state.GetSnapshot() // MODIFIED: Ignore new fields
    maxNameLen := 0
    maxAcronymLen := 0
