...
func handleCommand(ctx context.Context, state *PollerState, cmd interface{}, commandPacketChan chan<- []byte, soeLogger *log.Logger) {
    ...
    switch c := cmd.(type) {
    case SetBitCmd:
        current, _, _, _, _, _, _, _, _, _ := state.GetSnapshot() // MODIFIED: Ignore CommandConnCount and LastCommand
...
func processStateChanges(state *PollerState, log *log.Logger) {
    current, previous, _, _, _, _, _, _, _, _ := state.GetSnapshot() // MODIFIED: Ignore CommandConnCount and LastCommand
    if len(previous) == 0 {
        return
    }
    changeTime := time.Now()
...
func RunCommandListener(ctx context.Context, wg *sync.WaitGroup, state *PollerState, port int, logger *log.Logger) {
    ...
    for {
        ...
        default:
            ...
            connsMu.Lock()
            conns[conn] = struct{}{}
            state.UpdateCommandConnCount(len(conns)) // NEW: Update connection count
            connsMu.Unlock()
            connWG.Add(1)
            go func(conn net.Conn) {
                ...
                defer func() {
                    connsMu.Lock()
                    delete(conns, conn)
                    state.UpdateCommandConnCount(len(conns)) // NEW: Update on disconnect
                    connsMu.Unlock()
                }()
    ...
    for {
        ...
        default:
            ...
            if err != nil {
                ...
                state.UpdateLastCommand(cmdStr) // NEW: Store last command
            }
