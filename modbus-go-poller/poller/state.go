...
type PollerState struct {
    ...
    CommandConnCount int    // NEW: Track number of active command connections
    LastCommand string     // NEW: Store the last command received
}
...
func NewPollerState(dbEventChan chan<- database.Event, ioResultChan chan IO_Result, appConfig *config.AppConfig) *PollerState {
    ps := &PollerState{
        ...
        lastHeartbeatTime: 0,
        CommandConnCount: 0, // NEW: Initialize to 0
        LastCommand: "",     // NEW: Initialize to empty string
    }
...
func (ps *PollerState) GetSnapshot() (map[uint16]uint16, map[uint16]uint16, map[uint16]time.Time, map[string]ActiveAlarm, TxRxInfo, TxRxInfo, TimingInfo, string, int, string) { // MODIFIED: Added CommandConnCount and LastCommand to return
    ps.mu.Lock()
    defer ps.mu.Unlock()
    ...
    return current, previous, lastChange, alarms, ps.LastTx, ps.LastRx, ps.Timing, ps.Status, ps.CommandConnCount, ps.LastCommand // MODIFIED: Return new fields
}
...
func (ps *PollerState) UpdateCommandConnCount(count int) { // NEW: Method to update command connection count
    ps.mu.Lock()
    defer ps.mu.Unlock()
    ps.CommandConnCount = count
}

func (ps *PollerState) UpdateLastCommand(cmd string) { // NEW: Method to update last command
    ps.mu.Lock()
    defer ps.mu.Unlock()
    ps.LastCommand = cmd
}